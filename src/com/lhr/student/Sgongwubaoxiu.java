...
class Sgongwubaoxiu extends JFrame implements ActionListener {
	...
	JLabel JL = new JLabel("公物报修", JLabel.CENTER);// 使用文本创建一个标签对象

	JLabel JLDno = new JLabel("宿舍号:"); // 使用文本创建一个标签对象
	JTextField JTDno = new JTextField(); // 创建一个文本框对象

	JLabel JLName = new JLabel("报修物品:"); // 使用文本创建一个标签对象
	JTextField JTName = new JTextField(); // 创建一个文本框对象

	JLabel JLno = new JLabel("物品号:"); // 使用文本创建一个标签对象
	JTextField JTno = new JTextField(); // 创建一个文本框对象

	JLabel JLSdate = new JLabel("提交日期:"); // 使用文本创建一个标签对象
	JTextField JTSdate = new JTextField(); // 创建一个文本框对象

	JLabel JLcause = new JLabel("报修原因:"); // 使用文本创建一个标签对象
	...
	public Sgongwubaoxiu() // 创建AddStudent构造函数
	{
		this.setTitle("学生—物品报修"); // 设置窗口标题
		...
		JLSdate.setBounds(100, 105, 60, 20); // 设置学院标签的初始位置
		this.add(JLSdate); // 将学院标签添加到容器
		JTSdate.setBounds(200, 105, 100, 20); // 设置文本框的初始位置
		this.add(JTSdate); // 将文本框添加到容器

		JLName.setBounds(100, 130, 60, 20); // 设置姓名标签的初始位置
		this.add(JLName); // 将姓名标签添加到容器
		JTName.setBounds(200, 130, 100, 20); // 设置文本框的初始位置
		this.add(JTName); // 将文本框添加到容器

		JLno.setBounds(100, 155, 60, 20); // 设置班级标签的初始位置
		this.add(JLno); // 将班级标签添加到容器
		JTno.setBounds(200, 155, 100, 20); // 设置文本框的初始位置
		...
		this.setBounds(500, 330, 430, 430); // 设置窗口尺寸大小
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == JBNext)// 处理"重置"事件
		{
			...
			JTSdate.setText(null);// 设置文本的text值为null
