...
class SxiugaiPassword extends JFrame implements ActionListener {
	...
	JLabel JL = new JLabel("学生密码修改", JLabel.CENTER);// 使用文本创建一个标签对象
	...
	public SxiugaiPassword() // 创建SetGrade构造函数
	{
		this.setTitle("学生—学生密码修改");// 设置窗口标题
		...
		JLPassword.setBounds(100, 150, 60, 20); // 设置班级标签的初始位置
		this.add(JLPassword); // 将班级标签添加到容器
		JTPassword.setBounds(200, 150, 100, 20); // 设置文本框的初始位置
		this.add(JTPassword); // 将文本框添加到容器

		JLPassword1.setBounds(100, 175, 60, 20); // 设置班级标签的初始位置
		this.add(JLPassword1); // 将班级标签添加到容器
		JTPassword1.setBounds(200, 175, 100, 20); // 设置文本框的初始位置
		this.add(JTPassword1); // 将文本框添加到容器

		JLRPassword1.setBounds(100, 200, 60, 20); // 设置班级标签的初始位置
		this.add(JLRPassword1); // 将班级标签添加到容器
		JTRPassword1.setBounds(200, 200, 100, 20); // 设置文本框的初始位置
		this.add(JTRPassword1); // 将文本框添加到容器

		JBSet.setBounds(70, 110, 70, 20);// //设置查询按钮的初始位置
		this.add(JBSet);// 将查询按钮添加到容器
		JBSet.addActionListener(this);// 给按钮添加监听器

		JBNext.setBounds(160, 110, 70, 20);// 设置重置按钮的初始位置
		this.add(JBNext);// 将重置按钮添加到容器
		JBNext.addActionListener(this);// 给按钮添加监听器

		jb.setBounds(249, 110, 70, 20);// 设置重置按钮的初始位置
	...
	public void actionPerformed(ActionEvent e) {
		...
		{
			String Sno = JTSno.getText();// 将文本框中包含的文本传给字符串sSno
