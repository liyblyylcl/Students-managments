...
class TWailaichaxun extends JFrame implements ActionListener {
	JLabel JL = new JLabel("外来人员查询", JLabel.CENTER);// 使用文本创建一个标签对象
	JLabel JLNumber = new JLabel("请输入证件号:");// 使用文本创建一个标签对象
	...
	public TWailaichaxun() // 创建SetGrade构造函数
	{
		this.setTitle("管理员—外来人员信息查询");// 设置窗口标题
		...
		JLNumber.setBounds(150, 80, 100, 20);// 设置学号标签的初始位置
		this.add(JLNumber);// 将学号标签添加到容器
		JTNumber.setBounds(250, 80, 90, 20);// 设置文本框的初始位置
	...
	public void actionPerformed(ActionEvent e) {
		...
		{
			String OIDno = JTNumber.getText();// 将文本框中包含的文本传给字符串snumber

			sql = "select * from  OutsideMen  where OIDno='" + OIDno + "'";// 检索出Id等于snumber的学生的所有信息
